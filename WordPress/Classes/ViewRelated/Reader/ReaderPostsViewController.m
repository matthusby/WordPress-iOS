...
static CGFloat const RPVCHeaderHeightPad = 40.0f;
...
// TODO: wireup the no results view.
// TODO: Automatically refresh if appropriate
// TODO: Saved scroll offset
// TODO: hide refresh control when scrolling
// TODO: when syncing, do not update rows until scrolling has ended.
// TODO: when syncing, reload and reset scroll offset instead of letting new content transition. Remember to account for refresh control if its showing
// TODO: when loading more / backfilling new rows can transition in.
// TODO: when content changes, update cached row heights.  Always update heights above the current visible rows.  No need to update heights for rows not scrolled in to view, these can still be estimated.
// TODO: When rotating update all estimated row height cos the offset WILL change.
...
// TODO: Title should be updated when topic loads, not when view appears
    [self updateTitle];
...
//    if (!self.tableViewHandler.resultsController || (self.resultsController.fetchedObjects.count > 0)) {
//        return;
//    }
...
- (NSString *)entityName
{
    return NSStringFromClass([ReaderPost class]);
}
